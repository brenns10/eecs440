...
def print_fold_statistics(train_X, train_y, eval_X, eval_y):
    """Displays information about the contents of your folds."""
    print 'Fold statistics:'
    for tX, ty, eX, ey in zip(train_X, train_y, eval_X, eval_y):
        print '=> Train set size: ', len(tX)
        print '   Eval set size: ', len(eX)
        for l in np.unique(ty):
            print '   Train labels ', l, ': ', len(ty[ty==l])
            print '   Eval labels ', l, ': ', len(ey[ey==l])


def print_part_statistics(partitions_X, partitions_y):
    """Displays information about the contents of your partitions."""
    print 'Partition statistics:'
    for X, y in zip(partitions_X, partitions_y):
        print '=> Partition size: ', len(X)
        for l in np.unique(y):
            print '   Partition labels ', l, ': ', len(y[y==l])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_directory',
                        dest='dataset_directory', metavar='DIR', default='.')
    parser.add_argument('--k', metavar='FOLDS', type=int, default=5)
    parser.add_argument('--dataset', metavar='DATASET', default='voting')
    parser.add_argument('--meta_algorithm', metavar='ALG', required=False,
                        choices=['bagging', 'boosting'], help='Bagging or boosting, if desired')
    parser.add_argument('--meta_iters', metavar='N', type=int, required=False,
                        help='Iterations for bagging or boosting, if applicable')
    parser.add_argument('--fs_algorithm', metavar='ALG', required=False,
                        choices=['pca'], help='Feature selection algorithm, if desired')
    parser.add_argument('--fs_features', metavar='N', required=False,
                        type=int, help='Number of feature to select, if applicable')

    subparsers = parser.add_subparsers(
        dest='classifier', help='Classifier options')

    dtree_parser = subparsers.add_parser('dtree', help='Decision Tree')
    dtree_parser.add_argument(
        '--depth', type=int, help='Maximum depth for decision tree, 0 for None', default=2)

    ann_parser = subparsers.add_parser('ann', help='Artificial Neural Network')
    ann_parser.add_argument('--gamma', type=float,
                            help='Weight decay coefficient', default=0.01)
    ann_parser.add_argument('--layer_sizes', type=int,
                            help='Number of hidden layers', default=3)
    ann_parser.add_argument('--num_hidden', type=int,
                            help='Number of hidden units in the hidden layers', default=40)
    ...
    svm_parser = subparsers.add_parser(
        'linear_svm', help='Linear Support Vector Machine')
    svm_parser.add_argument(
        '--c', type=float, help='Regularization parameter for SVM', default=1)

    nbayes_parser = subparsers.add_parser('nbayes', help='Naive Bayes')
    nbayes_parser.add_argument(
        '--alpha', type=float, help='Smoothing parameter for Naive Bayes', default=1)

    lr_parser = subparsers.add_parser(
        'logistic_regression', help='Logistic Regression')
    lr_parser.add_argument(
        '--c', type=float, help='Regularization parameter for Logistic Regression', default=10)
